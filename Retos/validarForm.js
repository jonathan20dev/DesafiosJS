...
const validateForm = (formData, registeredUsers) => {
    if (formData.name && formData.password && formData.email && formData.password) {
        const verificarMail = registeredUsers.find(user => user.email === formData.email)
        if (verificarMail === undefined) {
